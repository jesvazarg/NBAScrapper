...
class FoulPlay(Play):
    def __init__(self, game_id: str = "", team_id: str = "", quarter: int = 1, play_time: str = "12:00", text: str = "",
                 id: str = "", type: str = "", player_id: str = "", drawn_id: str = ""):
        super().__init__(game_id, team_id, quarter, play_time, text, id)
        self.type = str(type)
        self.player_id = str(player_id) if player_id is not None else None
        self.drawn_id = str(drawn_id) if drawn_id is not None else None

    def __str__(self):
        res = "FoulPlay -> " + super().__str__() + ", type: " + str(self.type)
        if self.player_id != "":
            res += ", player_id: " + self.player_id
        if self.drawn_id != "":
            res += ", drawn_id: " + self.drawn_id

        return res

    def init_with_play(self, play: Play, type: str = "", player_id: str = "", drawn_id: str = ""):
